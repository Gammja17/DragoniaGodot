...
## 다 돌면 퀘스트 m0 의 '마을을 둘러본다' 대목이 넘어가고, 엘더에게 돌아가면 엘더가 보는 앞에서 허수아비를 깬 뒤 첫 일거리를 받는다.
##
...
static func _end() -> void:
	...
	Quests.notify("tour")       # m0: 마을을 둘러본다 → 엘더에게 돌아간다 → 엘더가 보는 앞에서 허수아비 (구경이 끝나자마자 허수아비부터 시켜서 당황스러웠다)
	Save.save_game()
